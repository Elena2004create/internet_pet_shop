...
def get_connection(request: Request, user = Depends(get_current_user), password = Depends(get_password)):
    conf: Config = request.app.state.config
    conn = connect(f"postgresql://{conf.db_user}:{conf.db_pass}@{conf.db_host}:{conf.db_port}/{conf.db_name}", row_factory=dict_row)
    ...
    if user.role == "ADMIN":
        conn.close()
        conn = connect(f"postgresql://{user.name}:{password}@{conf.db_host}:{conf.db_port}/{conf.db_name}", row_factory=dict_row)
        yield conn
    else:
        yield conn
    conn.close()
